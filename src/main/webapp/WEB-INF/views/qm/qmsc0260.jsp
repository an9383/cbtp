	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">부적합관리대장조회(레이저)</li>
			</ol>
		</nav>
	</header>
								...
								<table id="unfitAdmTable" class="table table-bordered tr_highlight_row">
									...
									<tfoot class="thead-light">
										<tr>
											...
											<th class="totalFaultyQty" style="background-color: #a9c9f0; color: black;"></th>
											<th colspan='4' style="background-color: #a9c9f0; color: black;"></th>
											<th class="totalUnfitFaultyCnt" style="background-color: #a9c9f0; color: black;"></th>
											<th colspan='4' style="background-color: #a9c9f0; color: black;"></th>
										</tr>
									</tfoot>
								</table>
...
<script>
	let currentHref = "qmsc0260"; 
	let currentPage = $('.' + currentHref).attr('id');
	
	$('#'+currentPage).closest('.has-child','li').addClass('has-open has-active');
	$('#'+currentPage).closest('.menu-item').addClass('has-active');    
	$(document).attr("title","부적합관리대장조회(레이저)"); 
	...
	let unfitAdmTable = $('#unfitAdmTable').DataTable({
		...
		columns : [
			...
			{//수량
				...
				render : function(data, type, row, meta){
					if(data != null){
						if(row['unfitProc']=="001" || row['unfitProc']=="002"){
	    ...
	    drawCallback: function() {
			...
			let totalFaultyQty = $('#unfitAdmTable').DataTable().column(6,{ page: 'current'} ).data().sum();
			let totalUnfitFaultyCnt = $('#unfitAdmTable').DataTable().column(11,{ page: 'current'} ).data().sum();

			$('.totalFaultyQty').text(rmDecimal(totalFaultyQty));
			$('.totalUnfitFaultyCnt').text(rmDecimal(totalUnfitFaultyCnt));
			
			$('#unfitAdmTable').find('tfoot').remove();
	...
	unfitHtml += '<button type="button" class="btn btn-primary mr-1" id="btnAllRetv" onClick="btnAllRetvClick()">전체조회</button>';
	unfitHtml += '</div>';
	...
	selectBoxAppend(unfitApprArray, "unfitApprSelect", "", "1");
	...
	let faultyListTable = $('#faultyListTable').DataTable({
		...
		drawCallback: function(settings) {
			let totalFaulty = $('#faultyListTable').DataTable().column(1,{ page: 'current'} ).data().sum();
			
			$('#totalFaulty').text(totalFaulty);
        }, 
	});
