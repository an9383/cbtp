	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">출하검사결과조회(정밀가공)</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
...
<script>
	let currentHref = "qmsc0350";
	let currentPage = $('.' + currentHref).attr('id');

	$('#' + currentPage).closest('.has-child', 'li').addClass('has-open has-active');
	$('#' + currentPage).closest('.menu-item').addClass('has-active');
	$(document).attr("title","출하검사결과조회(정밀가공)"); 
	...
	let testResultTable = $('#testResultTable').DataTable({
	    ...
	    columnDefs : [ 
		    { targets : [9,10,11,12,13],render : $.fn.dataTable.render.number(','),className : 'td_top'},
