	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">자재정보관리(정밀가공)</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
								...
								<div class="table-responsive">									
									...
									<table class="table table-bordered" id="itemPartInfoTable">
										...
										<tr>
											...
											<th>안전재고</th>
											<td><input type="text" class="form-control" id="safetyStock" name="safetyStock" style="text-align:right;" disabled></td>
										</tr>
										<tr>
											<th>박스 단위</th>
											<td><input type="text" class="form-control" id="boxCount" name="boxCount" value="0" style="text-align:right;"></td>
											<th>소포장 단위</th>
											<td><input type="text" class="form-control" id="smallPackage" name="smallPackage" value="0" style="text-align:right;"></td>
										</tr>
										<tr>
											<th>*품목사용여부</th>
...
<script>
	...
	let currentHref = "bmsc0410";
	let currentPage = $('.' + currentHref).attr('id');

	$('#' + currentPage).closest('.has-child', 'li').addClass(
			'has-open has-active');
	$('#' + currentPage).closest('.menu-item').addClass('has-active');
	$(document).attr("title", "자재정보관리(정밀가공)");
		...
		$.ajax({
			...
			success : function(res) {
				...
				if (res.result == 'ok') {
					...
					$('#importDate').val(data.importDate!=null?moment(data.importDate).format('YYYY-MM-DD'):"");
...
// 						$('#tab4Nav').removeClass('disabled');
	...
	//숫자 콤마생성
	$(document).on('keyup', "#importCost, #exportCost, #boxCount, #smallPackage, #safetyStock",  function(evt) { //입력시 콤마 추가
		var costSum = 0;
		if ($(this).val() == "") {
			$(this).val("0");
			$(this).select();
