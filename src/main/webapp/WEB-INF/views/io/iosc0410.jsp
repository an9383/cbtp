	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">발주관리(정밀가공)</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
	<div class="container-fluid" id="main">
              ...
              <div class="card top_sortbox p-2">
               <div class="row">
                ...
                <!-- 일자 -->
                  <label class="input-label-md">소요량등록일자</label>
                  <div class="m-1">
                     <div class="input-sub-md">
	                  <div class="form-group input-sub m-0 row">
	                   <input class="form-control-md" type="text" id="mrpDateFrom" name="mrpDateFrom" disabled/>
	                   <button onclick="fnPopUpCalendar(mrpDateFrom,mrpDateFrom,'yyyy-mm-dd')"  class="btn btn-secondary input-sub-search-md" id="mrpDateFromCalendar" type="button">
                  ...
                  <div class="m-1">
                     <div class="input-sub-md">
	                  <div class="form-group input-sub m-0 row">
	                   <input class="form-control-md" type="text" id="mrpDateTo" name="mrpDateTo" disabled />
	                   <button onclick="fnPopUpCalendar(mrpDateTo,mrpDateTo,'yyyy-mm-dd')"  class="btn btn-secondary input-sub-search-md" id="mrpDateToCalendar" type="button">
	                    <span class="oi oi-calendar"></span>
	                   </button>
	                  </div>
                     </div>
									...
									<tr>
										...
										<td>
											<div class="row">
												...
												<button type="button" class="btn btn-secondary d-none" id="btnPoNoReset">새로고침</button>
											</div>
										</td>
										<th>공급사</th>
										<td>
						...
						<div class="table-responsive">
							<table id="purchaseOrderDtlTable" class="table table-bordered">
								...
								<thead>
									<tr>	
										<th>
											<input type="checkbox" class="form-control" id="btnAllCheck2">
										</th>
										<th>품번</th>
										<th>품명</th>
										<th>미발주량</th>
...
<script>
	...
	let currentHref = "iosc0410";
	let currentPage = $('.' + currentHref).attr('id');
	
	$('#'+currentPage).closest('.has-child','li').addClass('has-open has-active');
	$('#'+currentPage).closest('.menu-item').addClass('has-active');    
	$(document).attr("title","발주관리(정밀가공)"); 
	...
	var mrpDateFromCal =  "${serverDateFrom}";
	var mrpDateToCal =  "${serverDateTo}";
	var dlvDateCal =  "${serverDateTo}";

	var mainChargr =  "${userNumber}";
	var mainChargrNm =  "${userNm}";	
	var poStatusOption = '';		//발주상태 초기값 : 진행
	var dealGubun = "";
	...
	let purchaseOrderDtlTable = $('#purchaseOrderDtlTable').DataTable({
		...
		columns : [
			...
			{data : 'ordQty',
				render : function(data, type, row, meta){
					if(data!=null){
						return '<input type="text" class="form-control number-float0" name="ordQty" value="'+rmDecimal(data)+'" style="text-align:right;" >';		
						
					} else{
						return '<input type="text" class="form-control number-float0" name="ordQty" value="0" style="text-align:right;">';
					}	
				}
			},
			{data : 'itemUnit',
			...
			/* {data : 'ordUnitCost',
				render : function(data, type, row, meta){
					if(data!=null){
						return '<input type="text" class="form-control number-float0" name="ordUnitCost" value="'+rmDecimal(data)+'" style="text-align:right;">';		
						
					} else{
						return '<input type="text" class="form-control number-float0" name="ordUnitCost" value="" style="text-align:right;">';
					}	
				}
			}, */
			{data : 'dlvDate',
	...
	// 새로고침버튼 클릭
	$('#btnPoNoReset').on('click', function() {
	...
	$('#btnSave').on('click',function(){
		...
		if ($('input[name=ordQty]').val()=='0' || $('input[name=ordQty]').val()=='') {
			toastr.warning('확정수량을 입력헤 주세요.');
			check=false;
			return false;
		} 
			...
			$.ajax({
				...
				success : function(res) {
					...
					if (res.result == 'ok') {
						...
						//새로고침 버튼숨김
						$('#poNo').css('max-width','100%');
						$('#poNo').removeClass('mr-2');
						$('#btnPoNoReset').addClass('d-none');
						
						toastr.success('등록되었습니다.');
					} else if(res.result == "fail"){

						//새로고침 버튼처리
						$('#poNo').css('max-width','60%');
						$('#poNo').addClass('mr-2');
						$('#btnPoNoReset').removeClass('d-none');
		...
		if (purchaseOrderDtlPopupTable == null || purchaseOrderDtlPopupTable == undefined) {
			purchaseOrderDtlPopupTable = $('#purchaseOrderDtlPopupTable').DataTable({
				...
				columnDefs : [ 
					...
					{ targets: [4,6], render: $.fn.dataTable.render.number( ',' ) },
				],
				order : [ [ 0, 'asc' ] ],
				buttons : [],
			});
	...
	//숫자만 입력하게 처리
	$(document).on('keyup',"input[name=ordQty],input[name=ordUnitCost]", function(event){
		...
		$(this).val(rmDecimal($(this).val().replace(",", "")));
	});
