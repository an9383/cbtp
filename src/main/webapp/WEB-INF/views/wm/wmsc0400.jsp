	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">APS 제조실행계획(정밀가공)</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
				...
				<table class="table table-bordered m-0">
					...
					<thead class="thead-light">
						...
						<tr>
							...
							<th class="text-center align-middle">가공시간</th>
...
<!-- <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Modal -->
<script>
	let currentHref = "wmsc0400";
	let currentPage = $('.' + currentHref).attr('id');

	$('#' + currentPage).closest('.has-child', 'li').addClass('has-open has-active');
	$('#' + currentPage).closest('.menu-item').addClass('has-active');
	$(document).attr("title", "APS 제조실행계획(정밀가공)");
	$('#calendar').hide();
	$('#selectYear').hide();
	$('#selectMonth').hide();
			...
			$.contextMenu({ // 우클릭 메뉴 생성
				...
				callback: function(key, options) {
					...
					if(key == 'delete') { // 삭제버튼을 눌렀을 경우
						...
						$('#dtlOrdCnt').text(addCommas(parseInt(data.dtlOrdCnt)) + ' EA'); // 주문수량
						...
						$('#processTime').text(addCommas(parseInt(data.processTime))+' 초'); // 가공시간
						let workTime = parseInt(data.readyTime) + ((parseInt(data.processTime)/60) * parseInt(data.dtlOrdCnt));
	...
	function makeScheduleTbody() {
		...
		for(var i=0;i<equipList.length;i++) { // 설비목록별로 계획표 생성
			...
			html += '<tr id="CAPA_'+data.equipCd+'" style="height: 0px;">';
	...
	function makeScheduleModule(data, planDate, planStopLst) {
		...
		let workTime = parseInt(data.readyTime) + (parseInt(data.processTime)/60 * parseInt(data.dtlOrdCnt)); // 총 작업시간(분)
		if(isNaN(workTime)) {
			workTime = '';
		} else {
		...
		if(data.ordState != 'PD') { // 미배정이 아닐 경우
			...
			html += '			<td style="padding: 0px 5px; border: none; text-align: right;">'+addCommas(parseInt(data.dtlOrdCnt))+' EA</td>';
			html += '		</tr>';
			html += '		<tr>';
			...
			html += '				<span style="float: left">'+addCommas(parseInt(data.dtlOrdCnt))+' EA</span>';
				...
				Sortable.create(id, {
					...
					onEnd: function (e) { // 드롭 했을때
						...
						if(state) {
							...
							if(data != undefined) {
								let workTime = Math.round(parseInt(data.readyTime) + (parseInt(data.processTime)/60 * parseInt(data.dtlOrdCnt))); // 총 작업시간(분)
	...
	function workOrderAdmIns(data) {
		$.ajax({
			url: '<c:url value="/po/workOrderAdmFinishIns"/>',
            type: 'POST',
            async: false,
            data: {
	...
	$('#btnApsCal').on('click', function() {
		...
		setTimeout(function() {
			...
			_.forEach(nonAssignLst, function(v) {
				...
				obj.planState		= 'PD';
				obj.workTime		= Math.round(parseInt(v.readyTime) + (parseInt(v.processTime)/60 * parseInt(v.dtlOrdCnt)));
				apsList.push(obj);
			});
	...
	let apsListByDateTable = $('#apsListByDateTable').DataTable({
		...
		columns : [
			...
			{ data : 'processTime', className : 'text-right',
				render : function(data, type, row, meta) {
					if(data != '' && data != null) {
						return addCommas(parseFloat((parseFloat(data)/60).toFixed(2)))+'분';
					} else {
						return '-';
					}
				}
			...
			{ className : 'text-right',
				render : function(data, type, row, meta) {
					...
					let processTime = parseFloat((parseInt(row['processTime'])/60).toFixed(2));
