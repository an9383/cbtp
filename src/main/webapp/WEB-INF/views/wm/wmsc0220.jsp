	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">기간별 생산 현황(레이저)</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
...
<script>
	...
	$(document).attr("title", "기간별 생산 현황(레이저)");
	...
	let workStatusTable = $('#workStatusTable').DataTable({
		...
		columns : [
			...
			{ data : 'cutSize', className : 'text-center' }, //CUT SIZE
			{ data : 'itemColor', className : 'text-center' }, //PNL SIZE
			...
			{ data : 'faultyTypeNm', className : 'text-center' }, //불량유형 
			...
			for(var i=0;i<data.length;i++) {
				...
				if(i+1 != data.length) { // 마지막 데이터가 아닐 경우
					if(moment(data[i].workStartDate).format('YYYYMMDD') != moment(data[i+1].workStartDate).format('YYYYMMDD')) { // 지시일자가 다를 경우
						let html = '';
							html += '<tr>';
							html += '	<th class="text-center" colspan="8" style="background-color:#aed3ec">생산일별소계</th>';
						...
						dateNoTimeSum = 0;
						 ...
						 /* if(equipCount != 0) {
							html = '';
								html += '<tr>';
								html += '	<th class="text-center" colspan="8" style="background-color:#bcecae">설비별소계</th>';
					...
					html2 += '<th colspan="8" style="background-color: #edacb1; color: #ff0000;">전체합계</th>';
