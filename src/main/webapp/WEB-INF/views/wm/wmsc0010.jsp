	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">생산지시현황(레이저)</li>
			</ol>
		</nav>
	</header>
...
<script>
	...
	$(document).attr("title", "생산지시현황(레이저)"); 
	...
	let contNoListTable = $('#contNoListTable').DataTable({
		...
		createdRow : function( row, data, dataIndex ) {
		    if (data['outputQty']!=null && parseInt(data['outputQty'])>0) {
		    	$(row).find('td').eq(7).css('color','blue').css('font-weight','bold');
		    }

		    if (data['faultyQty']!=null && parseInt(data['faultyQty'])>0) {
	...
	let workOrdListTable = $('#workOrdListTable').DataTable({
		...
		createdRow : function( row, data, dataIndex ) {
		    if (data['outputQty']!=null && parseInt(data['outputQty'])>0) {
		    	$(row).find('td').eq(11).css('color','blue').css('font-weight','bold');
		    }

		    if (data['faultyQty']!=null && parseInt(data['faultyQty'])>0) {
		    	$(row).find('td').eq(12).css('color','red').css('font-weight','bold');
		    }
		},
		columnDefs: [
	           {"targets": "_all", "className": "text-center"},
